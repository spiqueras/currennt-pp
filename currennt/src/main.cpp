...
#include "../../currennt_lib/src/optimizers/AdamOptimizer.hpp"
#include "../../currennt_lib/src/optimizers/AdamaxOptimizer.hpp"
...
std::shared_ptr<data_sets::DataSet> loadDataSet(data_set_type dsType);
...
int trainerMain(const Configuration &config)
{
    try {
        ...
        // load data sets
        auto trainingSet    = std::make_shared<data_sets::DataSet>();
        auto validationSet  = std::make_shared<data_sets::DataSet>();
        auto testSet        = std::make_shared<data_sets::DataSet>();
        auto feedForwardSet = std::make_shared<data_sets::DataSet>();
        ...
        int inputSize = -1;
        int outputSize = -1;
        inputSize = trainingSet->inputPatternSize() * (1 + config.inputLeftContext() + config.inputRightContext());
        ...
        if (config.trainingMode()) {
            ...
            fflush(stdout);
            std::unique_ptr<optimizers::Optimizer<TDevice> > optimizer;

            optimizers::SteepestDescentOptimizer<TDevice> *sdo;
            optimizers::AdamOptimizer<TDevice> *ao;
            optimizers::AdamaxOptimizer<TDevice> *amo;
            ...
            case Configuration::OPTIMIZER_STEEPESTDESCENT:
                sdo = new optimizers::SteepestDescentOptimizer<TDevice>(
                    ...
                    config.learningRate(), config.momentum(), config.weight_decay()
                    );
                optimizer.reset(sdo);
                break;
            case Configuration::OPTIMIZER_ADAM:
                ao = new optimizers::AdamOptimizer<TDevice>(
                    neuralNetwork, *trainingSet, *validationSet, *testSet,
                    config.maxEpochs(), config.maxEpochsNoBest(), config.validateEvery(), config.testEvery(),
                    config.learningRate(), config.adam_beta1(), config.adam_beta2(), config.adam_epsilon(),
                    config.weight_decay()
                    );
                optimizer.reset(ao);
                break;
            case Configuration::OPTIMIZER_ADAMAX:
                amo = new optimizers::AdamaxOptimizer<TDevice>(
                    neuralNetwork, *trainingSet, *validationSet, *testSet,
                    config.maxEpochs(), config.maxEpochsNoBest(), config.validateEvery(), config.testEvery(),
                    config.learningRate(), config.adam_beta1(), config.adam_beta2(), config.adam_epsilon(),
                    config.weight_decay()
            ...
            while (!finished) {
                ...
                double duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime).count() / 1000.0;
            ...
            if (trainingSet != std::shared_ptr<data_sets::DataSet>())
                boost::filesystem::remove(trainingSet->cacheFileName());
            if (validationSet != std::shared_ptr<data_sets::DataSet>())
                boost::filesystem::remove(validationSet->cacheFileName());
            if (testSet != std::shared_ptr<data_sets::DataSet>())
        ...
        // evaluation mode
            ...
            if (config.feedForwardFormat() == Configuration::FORMAT_SINGLE_CSV) {
                ...
                int fracIdx = 0;
                std::shared_ptr<data_sets::DataSetFraction> frac;
            ...
            if (feedForwardSet != std::shared_ptr<data_sets::DataSet>()) 
...
int main(int argc, const char *argv[])
{
    ...
    if (config.useCuda()) {
      ...
      /* 
        int deviceCount;
        if ((err = cudaGetDeviceCount(&deviceCount)) != cudaSuccess) {
            std::cerr << "FAILED: " << cudaGetErrorString(err) << std::endl;
            return err;
        }
        ...
        for(int device = 0; device < deviceCount; ++device){ 
          ...
          if ((err = cudaSetDevice(device)) != cudaSuccess) {
              // might be in use, check the next device
              std::cerr << "FAILED: " << cudaGetErrorString(err) << std::endl;
              continue;
          } else
...
std::shared_ptr<data_sets::DataSet> loadDataSet(data_set_type dsType)
{
    ...
    case DATA_SET_TRAINING:
        ...
        balanceData = Configuration::instance().balanceData();
        break;
    ...
    auto ds = std::make_shared<data_sets::DataSet>(
        filenames,
        Configuration::instance().parallelSequences(), fraction, truncSeqLength, 
        fracShuf, seqShuf, noiseDev, balanceData, cachePath);
...
void printOptimizer(const Configuration &config, const optimizers::Optimizer<TDevice> &optimizer)
{
    ...
    printf("Weight decay:              %g\n", (double)config.weight_decay());
    if (dynamic_cast<const optimizers::SteepestDescentOptimizer<TDevice>*>(&optimizer)) {
        printf("Optimizer type:            Steepest descent\n");
        printf("Learning rate:             %g\n", (double)config.learningRate());
        printf("Momentum:                  %g\n", (double)config.momentum());
        printf("\n");
    }
    else if (dynamic_cast<const optimizers::AdamOptimizer<TDevice>*>(&optimizer)) {
        printf("Optimizer type:            Adam\n");
        printf("Learning rate:             %g\n", (double)config.learningRate());
        printf("Beta1:                     %g\n", (double)config.adam_beta1());
        printf("Beta2:                     %g\n", (double)config.adam_beta2());
        printf("Epsilon:                   %g\n", (double)config.adam_epsilon());
        printf("\n");
    }
    else if (dynamic_cast<const optimizers::AdamaxOptimizer<TDevice>*>(&optimizer)) {
        printf("Optimizer type:            Adamax\n");
        printf("Learning rate:             %g\n", (double)config.learningRate());
        printf("Beta1:                     %g\n", (double)config.adam_beta1());
        printf("Beta2:                     %g\n", (double)config.adam_beta2());
        printf("Epsilon:                   %g\n", (double)config.adam_epsilon());
    ...
    if (config.nesterovMomentum())
        printf("Nesterov's gradient        ON\n");
    else
        printf("Nesterov's gradient        OFF\n");
}
