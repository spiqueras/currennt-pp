        ...
        for (rapidjson::Value::ValueIterator layerChild = layersSection.Begin(); layerChild != layersSection.End(); ++layerChild) {
            ...
            try {
              layers::Layer<TDevice> *layer;
                ...
                else if (layerChild->HasMember("multi"))
                    layer = LayerFactory<TDevice>::createLayer("multi", &*layerChild, weightsSection, parallelSequences, maxSeqLength, m_layers.back().get(), layerType);
                else
                ...
                m_layers.push_back(std::shared_ptr<layers::Layer<TDevice> >(layer));
            }
...
template <typename TDevice>
const std::vector<std::shared_ptr<layers::Layer<TDevice> > >& NeuralNetwork<TDevice>::layers() const
{
    return m_layers;
}
...
std::vector<std::vector<std::vector<real_t> > > NeuralNetwork<TDevice>::getOutputs()
{
    ...
    for (int patIdx = 0; patIdx < static_cast<int>(ol.patTypes().size()); ++patIdx) {
