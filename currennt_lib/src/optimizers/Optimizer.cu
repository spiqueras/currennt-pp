...
namespace optimizers {
    ...
    real_t Optimizer<TDevice>::_processDataSet(data_sets::DataSet &ds, bool calcWeightUpdates, real_t *classError)
    {
        ...
        *classError = (real_t) ds.totalTimesteps();
        ...
        while ((frac = ds.getNextFraction())) {
            ...
            if (layers::BinaryClassificationLayer<TDevice> *po_layer = dynamic_cast<layers::BinaryClassificationLayer<TDevice>*>(&m_neuralNetwork.postOutputLayer()))
                *classError -= po_layer->countCorrectClassifications();
            else if (layers::MulticlassClassificationLayer<TDevice> *po_layer = dynamic_cast<layers::MulticlassClassificationLayer<TDevice>*>(&m_neuralNetwork.postOutputLayer()))
                *classError -= po_layer->countCorrectClassifications();
    ...
    template <typename TDevice>
    void Optimizer<TDevice>::_injectGradientNoise(real_t sigma) 
    {
        // generate vector of gradient noise on the host
        ...
        for (auto &curWeightUpdate : m_curWeightUpdates) {
            Cpu::real_vector weightNoise(curWeightUpdate.size());
            for (int i = 0; i < weightNoise.size(); ++i) {
                weightNoise[i] = dist(*gen);
            }

            // copy weight noise to device
            real_vector weightNoiseD(curWeightUpdate.size());
            thrust::copy(weightNoise.begin(), weightNoise.end(), weightNoiseD.begin());

            // add weight noise to device vector of weights
            thrust::transform(curWeightUpdate.begin(), curWeightUpdate.end(), weightNoiseD.begin(), curWeightUpdate.begin(), thrust::plus<real_t>());
        }
    }
    ...
    bool Optimizer<TDevice>::train()
    {
        if (!m_finished) {
            ...
            if (Configuration::instance().learningRateDecay() != 1 &&
                  (m_epochsSinceLowestError && m_epochsSinceLowestError % Configuration::instance().epochsLRUpdate() == 0))
                _updateParameters();
