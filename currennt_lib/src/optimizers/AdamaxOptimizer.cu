...
namespace {
    ...
    struct UpdateWeightFn
    {
        ...
        __host__ __device__ real_t operator() (const int &weightIdx)
        {
            real_t newWeight;
            if (nag)
                newWeight = _updateWeightNadamaxFn(weightIdx);
            else
                newWeight = _updateWeightAdamaxFn(weightIdx);
            if (average)
        ...
        // Adamax
        __host__ __device__ real_t _updateWeightAdamaxFn (const int &weightIdx)
        {
            real_t nmt = beta1 * mt[weightIdx] + (1 - beta1) * weightUpdates[weightIdx];
            real_t nut = max(beta2 * ut[weightIdx], abs(weightUpdates[weightIdx]) + epsilon);  // Make sure it is not zero
            real_t mtc = nmt / (1 - helpers::TypedMath<real_t>::pow(beta1, t));  // This could be computed outside the kernel
            mt[weightIdx] = nmt;
            ut[weightIdx] = nut;
            real_t delta = - learningRate * mtc / nut;
        ...
        __host__ __device__ real_t _updateWeightNadamaxFn (const int &weightIdx)
        {
             // return _updateWeightAdamaxFn (weightIdx);
...
namespace optimizers {
    ...
    void AdamaxOptimizer<TDevice>::_updateWeights()
    {
        ...
        for (size_t i = 1; i < this->_neuralNetwork().layers().size()-1; ++i) {
            ...
            updateWeightFn.ut = helpers::getRawPointer(layerParams->m_uts);
    ...
    template <typename TDevice>
    AdamaxOptimizer<TDevice>::AdamaxOptimizer(
            ...
            if (layer) {
                ...
                if (ml) {
                    for (auto& name: ml->layerNames()) {
                        ...
                        lp->m_uts.resize(ml->weights().size(), 0);
                    ...
                    lp->m_uts.resize(layer->weights().size(), 0);
    ...
    template <typename TDevice>
    void AdamaxOptimizer<TDevice>::exportState(const helpers::JsonDocument &jsonDoc) const
    {
...
//        Optimizer<TDevice>::_exportWeights(jsonDoc, "steepest_descent_optimizer_weight_deltas", m_weightDeltas);
    ...
    template <typename TDevice>
    void AdamaxOptimizer<TDevice>::importState(const helpers::JsonDocument &jsonDoc)
    {
...
//        Optimizer<TDevice>::_importWeights(jsonDoc, "steepest_descent_optimizer_weight_deltas", &m_weightDeltas);
    ...
    template <typename TDevice>
    void AdamaxOptimizer<TDevice>::_updateParameters()
    {
    ...
    template class AdamaxOptimizer<Cpu>;
    template class AdamaxOptimizer<Gpu>;
