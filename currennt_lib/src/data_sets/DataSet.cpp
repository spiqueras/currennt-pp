...
namespace internal {
    ...
    bool comp_seqs_subtype(const data_sets::DataSet::sequence_t &a, const data_sets::DataSet::sequence_t &b)
    {
      return a.subType < b.subType;
    }

    bool comp_seqs_length(const data_sets::DataSet::sequence_t &a, const data_sets::DataSet::sequence_t &b)
    {
...
namespace data_sets {
    ...
    struct thread_data_t
    {
        ...
        std::function<std::shared_ptr<DataSetFraction> ()> taskFn;
        std::shared_ptr<DataSetFraction> frac;
        bool finished;
    };
    ...
    void DataSet::_nextFracThreadFn()
    {
        for (;;) {
            // wait for a new task
            std::unique_lock<std::mutex> lock(m_threadData->mutex);
            while (!m_threadData->taskFn && !m_threadData->terminate)
                m_threadData->cv.wait(lock);

            // terminate the thread?
            if (m_threadData->terminate) break;

            // execute the task
            m_threadData->frac.reset();
            m_threadData->frac = m_threadData->taskFn();
            m_threadData->finished = true;
            m_threadData->taskFn = nullptr;
    ...
    {
        std::shuffle(m_sequences.begin(), m_sequences.end(), rg);
    }

    void DataSet::_shuffleFractions()
    {
        std::shuffle(m_fracFirstSeqIdx.begin(), m_fracFirstSeqIdx.end(), rg);
    }

    void DataSet::_addNoise(Cpu::real_vector &v)
    {
        if (!m_noiseDeviation) return;

        static std::mt19937 *gen = NULL;
        if (!gen)
            gen = new std::mt19937(Configuration::instance().randomSeed());

        std::normal_distribution<real_t> dist((real_t)0, m_noiseDeviation);
    ...
    Cpu::real_vector DataSet::_loadInputsFromCache(const sequence_t &seq)
    {
        ...
        m_cacheFile.read(reinterpret_cast<char*>(v.data()), sizeof(real_t) * v.size());
    ...
    Cpu::int_vector DataSet::_loadTargetClassesFromCache(const sequence_t &seq)
    {
        ...
        m_cacheFile.seekg(seq.targetsBegin);
        m_cacheFile.read(reinterpret_cast<char*>(v.data()), sizeof(int) * v.size());
        assert (m_cacheFile.tellg() - seq.targetsBegin == v.size() * sizeof(int));

        return v;
    }

    std::shared_ptr<DataSetFraction> DataSet::_makeFractionTask(int firstSeqIdx)
    {
        ...
        bool use_cache = Configuration::instance().useCache();

        // printf("(%d) Making task firstSeqIdx=%d...\n", (int)m_sequences.size(), firstSeqIdx);
        std::shared_ptr<DataSetFraction> frac(new DataSetFraction);
        ...
        int num_seqs_in_frac = 0;
        int num_timesteps_in_frac = 0;
        // fill fraction sequence info
        for (size_t seqIdx = firstSeqIdx; seqIdx < firstSeqIdx + m_parallelSequences; ++seqIdx) {
            if (seqIdx < m_sequences.size() &&
                m_sequences[seqIdx].subType == m_sequences[firstSeqIdx].subType) {
                ...
                num_timesteps_in_frac += m_sequences[seqIdx].length;
                num_seqs_in_frac      += 1;
            }
        }
        frac->m_numTimesteps = num_timesteps_in_frac;
        frac->m_subType      = m_sequences[firstSeqIdx].subType;
        ...
        for (int i = 0; i < num_seqs_in_frac; ++i) {
            const sequence_t &seq = m_sequences[firstSeqIdx + i];

            // inputs
            Cpu::real_vector inputs;
            if (use_cache)
                inputs = _loadInputsFromCache(seq);
            else
                inputs = m_inputs[seq.originalSeqIdx];
            _addNoise(inputs);
            ...
            // target classes
            if (m_isClassificationData) {
                Cpu::int_vector targetClasses;
                if (use_cache)
                    targetClasses = _loadTargetClassesFromCache(seq);
                else
                    targetClasses = m_outputsClas[seq.originalSeqIdx];
                for (int timestep = 0; timestep < seq.length; ++timestep) {
                    int tgt = 0; // default class
            ...
            // outputs
                ...
                if (use_cache)
                    outputs = _loadOutputsFromCache(seq);
                else
                    outputs = m_outputsRegr[seq.originalSeqIdx];
                for (int timestep = 0; timestep < seq.length; ++timestep) {
                    ...
                    if (timestep >= output_lag) {
                        ...
                        for (int oi = 0; oi < m_outputPatternSize; ++oi)
                            frac->m_outputs[tgtStart + oi] = 1.0f;  // default value
                    }
                }
            }
            ...
            if (!m_outputWeights.empty()) {
                frac->m_outputWeights.resize(m_outputPatternSize);
                thrust::copy_n(m_outputWeights.begin(), m_outputPatternSize, frac->m_outputWeights.begin());
            }
        }
    ...
    std::shared_ptr<DataSetFraction> DataSet::_makeFirstFractionTask()
    {
        if (m_sequenceShuffling) _shuffleSequences();
        if (m_fractionShuffling) _shuffleFractions();

        return _makeFractionTask(m_fracFirstSeqIdx[0]);
    }
    ...
    DataSet::DataSet(const std::vector<std::string> &ncfiles, int parSeq, real_t fraction, int truncSeqLength, bool fracShuf, bool seqShuf, real_t noiseDev, bool balanceData, std::string cachePath)
        ...
        , m_curFracIdx       (-1)
    {
        ...
        bool has_type    = false;  // For shared layers
        int seq_no = 0;

        // read the *.nc files
        for (const auto &nc : ncfiles) {
            std::vector<sequence_t> sequences;

            if (ret = nc_open(nc.c_str(), NC_NOWRITE, &ncid))
                throw std::runtime_error(std::string("Could not open '") + nc + "': " + nc_strerror(ret));

            // extract the patterns from the *.nc file
            try {
                int maxSeqTagLength = internal::readNcDimension(ncid, "maxSeqTagLength");
                int maxSeqTypeLength;
                has_type = internal::hasNcDimension(ncid, "maxSeqTypeLength");
                if (has_type)
                    maxSeqTypeLength = internal::readNcDimension(ncid, "maxSeqTypeLength");
                ...
                nSeq = static_cast<int>((real_t)nSeq * fraction);
                nSeq = std::max(nSeq, 1);
                ...
                for (int i = 0; i < nSeq; ++i) {
                    ...
                    std::string subType;
                    if (! Configuration::instance().feedForwardSubtype().empty())
                        subType = Configuration::instance().feedForwardSubtype();
                    else if (has_type)
                        subType = internal::readNcStringArray(ncid, "seqTypes", i, maxSeqTypeLength);
                    else
                        subType = "0";
                    while (seqLength > 0) {
                        sequence_t seq;
                        seq.originalSeqIdx = seq_no;
                ...
                if (use_cache) {
                  for (auto& seq : sequences) {
                    ...
                    m_inputs.resize(seq_no);
                    if (m_isClassificationData)
                        m_outputsClas.resize(seq_no);
                    else
                        m_outputsRegr.resize(seq_no);
                    for (auto& seq : sequences) {
                        m_minSeqLength = std::min(m_minSeqLength, seq.length);
                        m_maxSeqLength = std::max(m_maxSeqLength, seq.length);
                        m_inputs[seq.originalSeqIdx] = internal::readNcPatternArray(ncid, "inputs", inputsBegin, seq.length, m_inputPatternSize);
                        if (m_isClassificationData)
                            m_outputsClas[seq.originalSeqIdx] = internal::readNcArray<int>(ncid, "targetClasses", targetsBegin, seq.length);
                        else
                            m_outputsRegr[seq.originalSeqIdx] = internal::readNcPatternArray(ncid, "targetPatterns", targetsBegin, seq.length, m_outputPatternSize);
                ...
                if (first_file) {
                    ...
                    // Same as before, but for output weights
                    try {
                        m_outputWeights = internal::readNcArray<real_t>(ncid, "outputWeights",  0, m_outputPatternSize);
                    }
                    catch (std::runtime_error& err) {
                        // Do nothing
                    }
        ...
        m_threadData->thread    = std::thread(&DataSet::_nextFracThreadFn, this);
//        m_threadData->thread.detach();

        m_totalSequences = m_sequences.size();

        // sort sequences by length and subtype
        if (Configuration::instance().trainingMode()) {
            std::stable_sort(m_sequences.begin(), m_sequences.end(), internal::comp_seqs_length);
            std::stable_sort(m_sequences.begin(), m_sequences.end(), internal::comp_seqs_subtype);
        }

        // This is unneeded in non-"balanced data" configuration mode,
        // but the cost should be negigible
        std::map< std::string, std::vector<int> > fracPerType;

        int seqIdx = 0;
        m_fracFirstSeqIdx.push_back(0);
        fracPerType[m_sequences[seqIdx].subType] = std::vector<int>();
        fracPerType[m_sequences[seqIdx].subType].push_back(0);

        while (seqIdx < m_totalSequences) {
            if (m_sequences[seqIdx].subType != m_sequences[m_fracFirstSeqIdx.back()].subType ||
                (seqIdx - m_fracFirstSeqIdx.back()) >= m_parallelSequences) {
                m_fracFirstSeqIdx.push_back(seqIdx);
                if (m_sequences[seqIdx].subType != m_sequences[m_fracFirstSeqIdx.back()].subType) {
                    fracPerType[m_sequences[seqIdx].subType] = std::vector<int>();
                    fracPerType[m_sequences[seqIdx].subType].push_back(seqIdx);
                } else {
                    fracPerType[m_sequences[seqIdx].subType].push_back(seqIdx);
                }
              }
        ...
        if (balanceData) {
          // Find the maximum
            for (const auto& frac : fracPerType)
                if (frac.second.size() > max_size)
                    max_size = frac.second.size();
            for (const auto& frac : fracPerType) {
                size_t numFrac    = frac.second.size();
                size_t newNumFrac = numFrac;
                while (numFrac + newNumFrac <= max_size) {
                    m_fracFirstSeqIdx.insert(m_fracFirstSeqIdx.end(), frac.second.begin(), frac.second.end());
                    newNumFrac += numFrac;
                }
                if (newNumFrac < max_size)
                    m_fracFirstSeqIdx.insert(m_fracFirstSeqIdx.end(), frac.second.begin(), frac.second.begin() + (max_size - newNumFrac));
            }
        }
        rg.seed(Configuration::instance().randomSeed());
    }
    ...
    DataSet::~DataSet()
    {
        ...
        if (m_threadData) {
            {{
                std::lock_guard<std::mutex> lock(m_threadData->mutex);
                m_threadData->terminate = true;
                m_threadData->cv.notify_one();
            }}
        if (m_threadData->thread.joinable()) m_threadData->thread.join();
        }
    }
    ...
    std::shared_ptr<DataSetFraction> DataSet::getNextFraction()
    {
        // initial work
        if (m_curFracIdx == -1) {
            std::unique_lock<std::mutex> lock(m_threadData->mutex);
            m_threadData->taskFn = std::bind(&DataSet::_makeFirstFractionTask, this);
            m_threadData->finished = false;
            m_threadData->cv.notify_one();
            m_curFracIdx = 0;
        }
        ...
        // get the fraction
        std::shared_ptr<DataSetFraction> frac;
        if (m_curFracIdx < (int)m_fracFirstSeqIdx.size()) {
            frac = m_threadData->frac;
            m_curFracIdx += 1;
            // start new task
            if (m_curFracIdx < (int)m_fracFirstSeqIdx.size())
                m_threadData->taskFn = std::bind(&DataSet::_makeFractionTask, this, m_fracFirstSeqIdx[m_curFracIdx]);
            else
                m_threadData->taskFn = std::bind(&DataSet::_makeFirstFractionTask, this);
