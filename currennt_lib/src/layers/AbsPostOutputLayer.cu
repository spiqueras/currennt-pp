...
#include "AbsPostOutputLayer.hpp"
...
namespace {
    ...
    struct ComputeOutputErrorFn
    {
        ...
        __host__ __device__ real_t operator() (const thrust::tuple<const real_t&, const real_t&, int> &t) const
        {
            ...
            // calculate the error
            real_t error = (actualOutput - targetOutput) >= 0 ? 1 : -1;
...
namespace layers {
    ...
    AbsPostOutputLayer<TDevice>::AbsPostOutputLayer(const helpers::JsonValue &layerChild, Layer<TDevice> &precedingLayer)
        : PostOutputLayer<TDevice>(layerChild, precedingLayer, precedingLayer.size())
    {
    }

    template <typename TDevice>
    AbsPostOutputLayer<TDevice>::~AbsPostOutputLayer()
    {
    }

    template <typename TDevice>
    const std::string& AbsPostOutputLayer<TDevice>::type() const
    {
        static const std::string s("abs");
        return s;
    }

    template <typename TDevice>
    real_t AbsPostOutputLayer<TDevice>::calculateError()
    {
        ...
        real_t abs_error = thrust::transform_reduce(
    ...
    template <typename TDevice>
    void AbsPostOutputLayer<TDevice>::computeForwardPass()
    {
    }

    template <typename TDevice>
    void AbsPostOutputLayer<TDevice>::computeBackwardPass()
    {
    ...
    template class AbsPostOutputLayer<Cpu>;
    template class AbsPostOutputLayer<Gpu>;
