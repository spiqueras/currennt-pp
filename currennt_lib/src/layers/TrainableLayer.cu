...
namespace layers {
        ...
        , m_learningRate           (layerChild->HasMember("learningRate") ? static_cast<real_t>((*layerChild)["learningRate"].GetDouble()) : -1)
    {
        ...
        // create random weights if no weights are given in the network file
            ...
            Configuration::distribution_type_t weightsDistributionType;
            real_t weightsDistributionUniformMin = layerChild->HasMember("weights_uniform_min") ?
                                                   static_cast<real_t>((*layerChild)["weights_uniform_min"].GetDouble()) :
                                                   config.weightsDistributionUniformMin();
            real_t weightsDistributionUniformMax = layerChild->HasMember("weights_uniform_max") ?
                                                   static_cast<real_t>((*layerChild)["weights_uniform_max"].GetDouble()) :
                                                   config.weightsDistributionUniformMax();
            real_t weightsDistributionNormalMean = layerChild->HasMember("weights_normal_mean") ?
                                                   static_cast<real_t>((*layerChild)["weights_normal_mean"].GetDouble()) :
                                                   config.weightsDistributionNormalMean();
            real_t weightsDistributionNormalSigma = layerChild->HasMember("weights_normal_sigma") ?
                                                    static_cast<real_t>((*layerChild)["weights_normal_sigma"].GetDouble()) :
                                                    config.weightsDistributionNormalSigma();
            if (layerChild->HasMember("weights_dist")) {
                // Per-layer configuration
                std::string weightsDistString = (*layerChild)["name"].GetString();
                if (weightsDistString == "normal")
                    weightsDistributionType = Configuration::DISTRIBUTION_NORMAL;
                else if (weightsDistString == "uniform")
                    weightsDistributionType = Configuration::DISTRIBUTION_UNIFORM;
            } else {
                weightsDistributionType = config.weightsDistributionType();
            }

            if (weightsDistributionType == Configuration::DISTRIBUTION_UNIFORM) {
                real_t range = weightsDistributionUniformMax - weightsDistributionUniformMin;
                std::uniform_real_distribution<real_t> dist(0, range);
                for (auto &weight : weights)
                    weight = dist(*gen) + weightsDistributionUniformMin;
            }
            else {
                std::normal_distribution<real_t> dist(weightsDistributionNormalMean, weightsDistributionNormalSigma);
                for (auto &weight : weights)
                    weight = dist(*gen);
            }
        }
    ...
    template <typename TDevice>
    typename TrainableLayer<TDevice>::real_vector& TrainableLayer<TDevice>::weightUpdates()
    {
        return m_weightUpdates;
    }
    ...
    void TrainableLayer<TDevice>::injectWeightNoise(real_t sigma) 
    {
        ...
        std::normal_distribution<real_t> dist(0.0f, sigma);
        auto f = std::bind(dist, *gen);

        Cpu::real_vector weightNoise(weights().size());
        std::generate(weightNoise.begin(), weightNoise.end(), f);
