...
#include "WeightedRmsePostOutputLayer.hpp"
...
namespace {
    ...
    struct ComputeOutputErrorFn
    {
        ...
        __host__ __device__ real_t operator() (const thrust::tuple<const real_t&, const real_t&, int> &t) const
        {
            ...
            int i = outputIdx % layerSize;

            // calculate the error
            real_t error = (actualOutput - targetOutput) * weights[i] / (layerSize*rmse);
            return error;
        }
    };
...
namespace layers {
    ...
    WeightedRmsePostOutputLayer<TDevice>::WeightedRmsePostOutputLayer(const helpers::JsonValue &layerChild, Layer<TDevice> &precedingLayer)
    ...
    template <typename TDevice>
    WeightedRmsePostOutputLayer<TDevice>::~WeightedRmsePostOutputLayer()
    {
    }

    template <typename TDevice>
    const std::string& WeightedRmsePostOutputLayer<TDevice>::type() const
    {
        static const std::string s("wrmse");
        return s;
    }

    template <typename TDevice>
    real_t WeightedRmsePostOutputLayer<TDevice>::calculateError()
    {
        ...
        fn.weights       = helpers::getRawPointer(this->outputWeights());
    ...
    template <typename TDevice>
    void WeightedRmsePostOutputLayer<TDevice>::computeForwardPass()
    {

    }

    template <typename TDevice>
    void WeightedRmsePostOutputLayer<TDevice>::computeBackwardPass()
    {
    ...
    template class WeightedRmsePostOutputLayer<Cpu>;
    template class WeightedRmsePostOutputLayer<Gpu>;
