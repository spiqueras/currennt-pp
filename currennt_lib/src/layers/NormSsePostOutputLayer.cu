...
#include "NormSsePostOutputLayer.hpp"
...
namespace {
    ...
    struct ComputeOutputErrorFn
    {
        ...
        __host__ __device__ real_t operator() (const thrust::tuple<const real_t&, const real_t&, int> &t) const
        {
            ...
            // calculate the error
            real_t error = (actualOutput - targetOutput) / numTimesteps;
...
namespace layers {
    ...
    NormSsePostOutputLayer<TDevice>::NormSsePostOutputLayer(const helpers::JsonValue &layerChild, Layer<TDevice> &precedingLayer)
        : PostOutputLayer<TDevice>(layerChild, precedingLayer, precedingLayer.size())
    {
    }

    template <typename TDevice>
    NormSsePostOutputLayer<TDevice>::~NormSsePostOutputLayer()
    {
    }

    template <typename TDevice>
    const std::string& NormSsePostOutputLayer<TDevice>::type() const
    {
        static const std::string s("normsse");
        return s;
    }

    template <typename TDevice>
    real_t NormSsePostOutputLayer<TDevice>::calculateError()
    {
    ...
    template <typename TDevice>
    void NormSsePostOutputLayer<TDevice>::computeForwardPass()
    {
    }

    template <typename TDevice>
    void NormSsePostOutputLayer<TDevice>::computeBackwardPass()
    {
        // calculate the errors
        internal::ComputeOutputErrorFn fn;
        fn.layerSize = this->size();
        fn.patTypes  = helpers::getRawPointer(this->patTypes());
        fn.numTimesteps = this->numTimesteps();
    ...
    template class NormSsePostOutputLayer<Cpu>;
    template class NormSsePostOutputLayer<Gpu>;
