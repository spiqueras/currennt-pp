...
Configuration::Configuration(int argc, const char *argv[])
{
    ...
    feedForwardOptions.add_options()
        ...
        ("ff_subtype",     po::value(&m_feedForwardSubtype),                                    "overrides the subtype of the input data file")
    ...
    trainingOptions.add_options()
        ...
        ("epochs_no_best_lr",   po::value(&m_epochsLRUpdate)   ->default_value(0),                     "number of epochs without improvement in order to update learning rate")
        ("lr_decay_rate",       po::value(&m_learningRateDecay)->default_value((real_t)1,  "1"),       "sets the decay rate for learning rate update (usually ]0,1])")
        ("min_learning_rate",   po::value(&m_minLearningRate)  ->default_value((real_t)1e-7, "1e-7"),  "sets the minimum learning rate")
        ("balance_data",        po::value(&m_balanceData)      ->default_value(false),                 "balance the number of samples the network sees for each type and epoch")
    ...
    optimizerOptions.add_options()
        ...
        ("learning_rate",       po::value(&m_learningRate)     ->default_value((real_t)1e-5, "1e-5"),   "(ALL) sets the learning rate")
        ("weight_decay",        po::value(&m_weight_decay)     ->default_value((real_t)0, "0"),         "(ALL) sets the weights exponential decay rate for temporal averaging")
        ("momentum",            po::value(&m_momentum)         ->default_value((real_t)0.9,  "0.9"),    "(SGE) sets the momentum for the steepest descent optimizer")
        ("nesterov_momentum",   po::value(&m_nesterovMomentum) ->default_value(false),                  "(SGE, ADAM) use nesterov momentum")
        ("adam_beta1",          po::value(&m_adam_beta1)       ->default_value((real_t)0.9, "0.9"),     "(ADAM, ADAMAX) sets the beta1 parameter")
        ("adam_beta2",          po::value(&m_adam_beta2)       ->default_value((real_t)0.999, "0.999"), "(ADAM, ADAMAX) sets the beta1 parameter")
        ("adam_epsilon",        po::value(&m_adam_epsilon)     ->default_value((real_t)1e-8, "1e-8"),   "(ADAM, ADAMAX) sets the epsilon parameter")
        ;
    ...
    dataFilesOptions.add_options()
        ...
        ("use_cache",         po::value(&m_useCache)          ->default_value(true),      "enables or disables the cache")
    ...
    visibleOptions.add(optimizerOptions);
    ...
    else if (optimizerString == "adam")
        m_optimizer = OPTIMIZER_ADAM;
    else if (optimizerString == "adamax")
        m_optimizer = OPTIMIZER_ADAMAX;
    else {
        std::cout << "ERROR: Invalid optimizer. Possible values: steepest_descent, rprop, adam, adamax." << std::endl;
        exit(1);
    }
...
real_t Configuration::weight_decay() const
{
    return m_weight_decay;
}

real_t Configuration::adam_beta1() const
{
    return m_adam_beta1;
}

real_t Configuration::adam_beta2() const
{
    return m_adam_beta2;
}

real_t Configuration::adam_epsilon() const
{
    return m_adam_epsilon;
}

bool Configuration::nesterovMomentum() const
{
    return m_nesterovMomentum;
}

int Configuration::epochsLRUpdate() const
{
    return m_epochsLRUpdate;
}

real_t Configuration::learningRateDecay() const
{
    return m_learningRateDecay;
}

real_t Configuration::minLearningRate() const
{
    return m_minLearningRate;
}

bool Configuration::balanceData() const
{
...
const std::string& Configuration::feedForwardSubtype() const
{
    return m_feedForwardSubtype;
}
